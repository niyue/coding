...
// for each word, there may be multiple ladders connected to it, if there is no path to the word previously, we need to compute its connected words and do BFS for these words
// if the word is visited previously, and if the current path is a minimum path too (it should be equal to the previous path we found to this word, it cannot be smaller than it since we are doing BFS search)
// in this case, we only need to add it to the minimum paths list so that we can get all the paths in the end by using combination
public class Solution {
    ...
    private Map<String, List<Ladder>> bfs(String start, String end, Set<String> dict) {
        ...
        while(!(wordQueue.size() == 1 && wordQueue.peek() == null)) {
            ...
            if(ladder == null) {
            	...
            	// if the path to the word is not found yet
    ...
    // find all one-letter-differed words in the dictionary
    private List<Ladder> transformableWords(Ladder from, Set<String> dict) {
    ...
    private boolean isOneLetterDiff(String start, String end) {
    	int count = 0;
